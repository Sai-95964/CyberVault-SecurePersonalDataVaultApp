...
import 'upload_file_reader_stub.dart'
    if (dart.library.io) 'upload_file_reader_io.dart';
...
class _UploadScreenState extends State<UploadScreen> {
  ...
  Future<void> _pickAndUpload() async {
    if (!MasterKeyService.instance.isUnlocked) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Session locked — log in again')),
        );
      }
      return;
    }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              kIsWeb
                  ? 'Could not read file in browser — try a smaller image or PDF'
                  : 'Could not read file',
            ),
          ),
        );
    ...
    final mimeType = _mimeTypeFromFileName(name);
  ...
  /// Web and desktop: prefer in-memory bytes from file_picker (`withData: true`).
  Future<Uint8List?> _readPickedFileBytes(PlatformFile file) async {
    if (file.bytes != null) {
      return Uint8List.fromList(file.bytes!);
    }
    if (kIsWeb || file.path == null) return null;
    return readFileBytesFromPath(file.path!);
  }
